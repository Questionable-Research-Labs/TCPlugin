...
object DiscordMessageListener : ListenerAdapter() {
    override fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        if (!event.author.isBot && message.startsWith("\\") && event.channel.id == discordChannelID) {
            val splits = listOf(*message.split(" ").toTypedArray())

            if (splits.size > 1 && splits[0] == "\\" + discordPrefix) {
                val command = splits[1]
                val args = splits.stream().skip(2).collect(Collectors.toList())
                when (command) {
                    "say" -> if (args.size > 0) {
                        plugin.server.broadcastMessage(args.joinToString(" "))
                    ...
                    "info" -> discord?.sendMessage(
                        EmbedBuilder()
                            .setTitle("Info for " + discordPrefix)
